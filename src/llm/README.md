...
- `cache.rs`: local prompt cache entries under `.wiki_craft/runtime/prompt_cache/`.
...
Runtime metrics derived from this layer are written to `.wiki_craft/runtime/metrics/latest.json`,
`.wiki_craft/runtime/metrics/events.jsonl`, and can be rendered with `wiki_craft metrics --prometheus`.
