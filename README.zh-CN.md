...
这些命令都从仓库根目录执行。桌面壳会在本机随机 `127.0.0.1` 端口启动 API server，并把 API 地址注入 React UI。默认读取 `wiki_craft.toml`；如果要 review 其他 workspace，可以在启动前设置 `WIKI_CRAFT_CONFIG=/path/to/wiki_craft.toml`。

GUI 和服务日志会和 LLM audit log 分开：

- `.wiki_craft/runtime/gui/events.jsonl`：桌面 UI 事件、action 失败、前端/Tauri 上报的本地 API 请求失败。
- `.wiki_craft/runtime/web/events.jsonl`：本地 Axum API 启动、候选 action job 生命周期等后端服务事件。
- `.wiki_craft/runtime/audit/events.jsonl`：只记录 LLM workflow 的模型/工具调用链。
...
桌面 GUI 暴露的是同一组动作：`Approve Summaries` 对应 `summaries_staged`，`Merge Diff` 对应 `diff_ready`，`Reject` 对应未批准候选。GUI 会用类似编辑器 diff 的颜色显示 `diff.md`，但不会改变批准语义。
...
- GUI 和服务诊断日志分别写入 `.wiki_craft/runtime/gui/` 与 `.wiki_craft/runtime/web/`；audit log 只保留给 LLM 调用链。
...
- `src/web.rs`：桌面 GUI 使用的本地 Axum JSON API。
- `frontend/`：React/Vite candidate review UI。
- `src-tauri/`：启动本地 API 并承载前端的 Tauri 桌面壳。
